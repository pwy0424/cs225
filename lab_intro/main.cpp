...
PNG brighten(PNG original, int amount){
	for (int i = 0; i < original.width(); i++){
		for (int j = 0; j < original.height(); j++){
			if (original(i,j)->red + amount <256) {original(i,j)->red += amount;}
			else {original(i,j)->red = 255;}
			if (original(i,j)->green + amount <256) {original(i,j)->green += amount;}
			else {original(i,j)->green = 255;}
			if (original(i,j)->blue + amount <256) {original(i,j)->blue += amount;}
			else {original(i,j)->blue = 255;}
		}
	}
