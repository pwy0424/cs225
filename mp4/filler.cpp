...
template <template <class RGBAPixel> class OrderingStructure>
animation filler::fill( PNG & img, int x, int y,
	...
	for(int i = 0; i < (img.width()*img.height()); i++) processed[i] = false;	
	...
	{
		//cout<<"no"<<endl;
		RGBAPixel temp = pixels.remove();
		int tempx = xcoord.remove();
		int tempy = ycoord.remove();

		RGBAPixel newColor = fillColor(tempx,tempy);
		//cout<<"321"<<endl;
		int d = ((temp.red - original.red)*(temp.red - original.red)) + ((temp.green - original.green)*(temp.green - original.green)) + ((temp.blue - original.blue)*(temp.blue - original.blue));
		//cout<<d<<endl;
		//cout<<"yes"<<endl;
		if ((!(processed[(tempx*img.height()+tempy)]))&&(d<= tolerance))
		{	
			//cout<<"???"<<endl;
			processed[(tempx*img.height()+tempy)] = true;
			//if (d <= tolerance)
			//{
				img(tempx,tempy)->red = newColor.red;
				img(tempx,tempy)->green = newColor.green;
				img(tempx,tempy)->blue = newColor.blue;
				n++;
			//}
				//cout<<"1"<<endl;
			if((tempx+1) < img.width())
			{
