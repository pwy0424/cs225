...
gradientColorPicker::gradientColorPicker( RGBAPixel fadeColor1, 
	...
	: Color1(fadeColor1), Color2(fadeColor2), R(radius), X(centerX), Y(centerY)
{
	/** 
...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
	...
	{
		color.red = Color1.red + ((d*Color2.red)/R) - ((d*Color1.red)/R);
		color.green = Color1.green + ((d*Color2.green)/R) - ((d*Color1.green)/R);
		color.blue = Color1.blue + ((d*Color2.blue)/R) - ((d*Color1.blue)/R);
	}
	/**
