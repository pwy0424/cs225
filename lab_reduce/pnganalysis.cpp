...
map<RGBAPixel, int> pnganalysis::frequency( const PNG & image ) {
	...
	#pragma omp parallel
	{
		map<RGBAPixel, int> freq = ret_freq;
		#pragma omp for
		for( int i = 0; i < image.width(); ++i ) {
		...
		#pragma omp critical
		{
			...
			for( curr = freq.begin(), end = freq.end(); curr != end; ++curr ) {
    			pair<RGBAPixel, int> current_pair = *curr;
				ret_freq[current_pair.first] += current_pair.second;
			}
		}
	}
	return ret_freq;
