...
PNG ImageTools::verticalFlip(const PNG & source)
{
    ...
    for(int i = 0; i < width; ++i)
    {
        ...
        {
             int red   =  source(i, j)->red;
                output(width-i-1, j)->red = red;

	        int blue   =  source(i, j)->blue;
                output(width-i-1, j)->blue = blue;

	        int green   =  source(i, j)->green;
                output(width-i-1, j)->green = green;
        }
    }
